...
async function testLocationEndpoints() {
  ...
  try {
    ...
    const locationsResponse = await fetch(`${BASE_URL}/locations?limit=5&days=30`, {
      method: 'GET',
      headers
    });
    ...
    const analyticsResponse = await fetch(`${BASE_URL}/location-analytics?days=7`, {
      method: 'GET',
      headers
    });
...
async function generateSampleData() {
  ...
  console.log('💡 To populate real data, perform login requests from different locations');
}

      ...
      dashboardData = {
        ...
        securityScore: {
          ...
          factors: {
            ...
            locationSecurity:
							threatAssessment.metrics?.countryCount < 3 ? 85 : 65,
            behaviorAnalysis:
							threatAssessment.metrics?.failureRate < 5 ? 92 : 55,
          },
        },
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const assessment = await SecurityService.performThreatAssessment(
      userId,
      parseInt(timeframe),
    );
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          assessment,
          "Threat assessment completed successfully",
        ),
      );
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!ip) {
      return res
        .status(400)
        .json(new ApiResponse(400, null, "IP address is required"));
    }

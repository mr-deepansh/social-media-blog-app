...
class SecurityService {
	...
	static calculateRiskScore(activityData) {
		...
		if (factors.failedAttempts > 10) riskScore += 40;
		else if (factors.failedAttempts > 5) riskScore += 25;
		else if (factors.failedAttempts > 2) riskScore += 10;

		// IP diversity risk (0-25 points)
		if (factors.uniqueIPs > 10) riskScore += 25;
		else if (factors.uniqueIPs > 5) riskScore += 15;
		else if (factors.uniqueIPs > 3) riskScore += 8;

		// Device diversity risk (0-20 points)
		if (factors.deviceCount > 5) riskScore += 20;
		else if (factors.deviceCount > 3) riskScore += 12;
		else if (factors.deviceCount > 2) riskScore += 6;

		// Geographic anomalies (0-15 points)
		if (factors.locationChanges > 5) riskScore += 15;
		else if (factors.locationChanges > 2) riskScore += 8;
	...
	static generateSecurityRecommendations(analytics) {
		...
		if (analytics.threatIntelligence?.successRate < 80) {
			recommendations.push({
				...
				description:
					"High failure rate detected. Enable MFA to enhance account security.",
				action: "enable_mfa",
			});
		}
		...
		if (analytics.threatIntelligence?.uniqueIPCount > 10) {
			recommendations.push({
				...
				description:
					"Multiple IPs detected. Consider implementing IP whitelisting.",
		...
		if (
			analytics.geographicRisk?.some((country) => country.riskLevel === "high")
		) {
			recommendations.push({
				type: "geographic",
				priority: "high",
				...
				description:
					"High-risk countries detected. Consider geographic restrictions.",
		...
		if (analytics.suspiciousActivity?.length > 5) {
			recommendations.push({
				...
				description:
					"Multiple suspicious activities detected. Enable enhanced monitoring.",

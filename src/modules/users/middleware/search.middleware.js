...
export const advancedSearchPermission = (req, res, next) => {
  ...
  if (includePrivate === "true" && userRole !== "admin") {
    throw new ApiError(
      403,
      "Insufficient permissions to search private profiles",
    );
  }
  ...
  if (includeInactive === "true" && userRole !== "admin") {
    throw new ApiError(
      403,
      "Insufficient permissions to search inactive users",
    );
  }
...
export const searchLoggingMiddleware = (req, res, next) => {
  ...
  // Log search attempt
  console.log(
		`[SEARCH] ${req.user?.username || "Anonymous"} searching: "${req.query.search || req.query.q}"`,
  );

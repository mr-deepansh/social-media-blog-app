...
import { Post, Comment, Like, Share, Bookmark, View, Repost } from "../models/index.js";
...
class BlogService {
	...
	async createPost(data, userId) {
		...
		try {
			...
			return await Post.findById(post[0]._id).populate("author", "fullName username avatar").populate("media");
		} catch (error) {
	...
	async toggleLike(postId, userId) {
		...
		try {
			...
			if (existingLike) {
				...
				await Post.findByIdAndUpdate(postId, { $inc: { "engagement.likeCount": -1 } }, { session });
			} else {
				await Like.create(
					[
						{
	...
	async addComment(postId, userId, content, parentCommentId = null) {
		...
		try {
			...
			await Post.findByIdAndUpdate(postId, { $inc: { "engagement.commentCount": 1 } }, { session });

			await session.commitTransaction();

			return await Comment.findById(comment[0]._id).populate("author", "fullName username avatar");
		} catch (error) {
			await session.abortTransaction();
			throw error;
		} finally {

...
class DashboardValidator {
  static validateAggregationResult(result) {
    ...
    const { basicCounts, roleDistribution, recentUsers, monthlyGrowth } =
			result;
...
class DashboardProcessor {
  static buildAggregationPipeline() {
    const monthsAgo = new Date(
      Date.now() -
				QUERY_CONFIG.MONTHLY_LOOKBACK_MONTHS * 30 * 24 * 60 * 60 * 1000,
    );
...
class DashboardCacheManager {
  static generateCacheKey() {
    const timeSlot = Math.floor(
      Date.now() / (CACHE_CONFIG.TIME_BUCKET_SECONDS * 1000),
    );
...
export const getAdminDashboard = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const processedData =
			DashboardProcessor.processAggregationResults(aggregationResult);
    ...
    const dashboard = {
      ...
      meta: {
        ...
        performanceGrade:
					DashboardProcessor.calculatePerformanceGrade(executionTime),
    ...
    const fallbackDashboard = await safeAsyncOperation(
      () => DashboardCacheManager.getFallbackDashboard(),
      null,
      false,
    );
    ...
    handleControllerError(
      ...
      req,
      res,
      Date.now() - Number(startTime) / 1000000,
      logger,
    );
  }
});

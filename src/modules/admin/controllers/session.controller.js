...
const getAdminSessionAnalytics = asyncHandler(async (req, res) => {
	...
	const sessionAnalytics =
		await sessionService.getAdminSessionAnalytics(adminId);
	...
	const response = {
		...
		summary: {
			...
			activeSessions: sessionAnalytics.reduce(
				(sum, admin) => sum + admin.activeSessions,
				0,
			),
			totalSessions: sessionAnalytics.reduce(
				(sum, admin) => sum + admin.totalSessions,
				0,
			),
			avgSessionDuration: Math.round(
				sessionAnalytics.reduce(
					(sum, admin) => sum + (admin.avgDuration || 0),
					0,
		...
		meta: {
			...
			performanceGrade:
				executionTime < 50 ? "A++" : executionTime < 100 ? "A+" : "A",
	...
	return res
		.status(200)
		.json(new ApiResponse(200, response, "Admin session analytics generated"));
});
...
const getAdminSessionDetails = asyncHandler(async (req, res) => {
		...
		return res
			...
			.json(
				new ApiResponse(
					200,
					response,
					"Admin has no session data - not logged in yet",
				),
			);
	}
	...
	return res
		.status(200)
		.json(new ApiResponse(200, response, "Admin session details retrieved"));
});
